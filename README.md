...
- **VLAN identification** — Full 802.1Q single-tag and 802.1ad QinQ double-tag parsing: extracts VLAN ID, PCP priority bits, DEI bit, and outer/inner VIDs for QinQ; tracked per host, per connection, and aggregated in stats. The VLAN Graph view shows VLANs as super-nodes with hosts clustered inside; multi-VLAN hosts (VLAN hoppers) are physically placed between their segments; cross-VLAN traffic highlighted in red. Clicking a VLAN super-node opens a detail panel showing PCP distribution, QinQ members, cross-VLAN partners with byte counts, and the full member host list. A stats bar summarises cross-VLAN flows, total cross-VLAN bytes, untagged hosts, and multi-VLAN / hopping host count.
