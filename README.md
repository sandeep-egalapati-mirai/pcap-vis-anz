...
- **Timeline** — Scrub or auto-play packet activity over time; packet-density minimap (rAF-throttled for smooth playback); speed selector (0.5×, 1×, 2×, 5×); abs/rel toggle switches between UTC HH:MM:SS and relative +Ns; Space to play/pause, ←/→ to step
...
- **Keyboard shortcuts** — Press `?` to open the overlay listing all shortcuts: `1`–`7` switch views, `F` fits graph, `/` focuses search, `Space`/`←`/`→` control timeline, `P` pins selected node, `Esc` closes panels
